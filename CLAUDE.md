...
**Learning-mode phase transitions**: the mentor LLM never decides when a phase is done (`mentor.md` explicitly tells it the app owns this). After each mentor turn, `streamConversation` (`server.go`) runs `Agents.CheckPhaseCompletion` scoped to the messages since the phase started (`learning.go`'s `messagesSincePhaseStart`); if it judges the phase goal met, the server auto-advances (`advancePhase`) and emits an SSE `phase_advanced` event that the client (`app.js`) turns into a full page reload — there's no htmx OOB-swap machinery in this codebase, so this event is a plain JSON `data:` frame like `token`/`usage`/`error`, not an HTML fragment. The "Этап готов" button (`POST /sessions/{id}/learning/advance`) always stays clickable and runs the same check; if it says the goal wasn't met, the phase still advances but is recorded `forced` with `notes` (see `store` above) — this fails open (treated as forced, not blocked) if the check itself errors, e.g. no API key, so the button can't get someone stuck.
