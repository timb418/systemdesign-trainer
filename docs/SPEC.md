...
### 5.5. Обучение с постепенным раскрытием

Использует тот же банк, но ведёт ученика через шесть явных этапов: ориентация, требования, оценка нагрузки, HLD, deep dive, рефлексия. На каждом этапе сначала обязательна собственная попытка. Помощь раскрывается лестницей: наводящий вопрос → указание области → объяснение концепции → более конкретная опора.

Этапом и уровнем подсказки управляет сервер, не модель наставника. Наставник получает публичный бриф, цель текущего этапа, учебные цели и доступ к фактам условия через `reveal_facts`, но не получает `preferred_solution` или gold diagram. Эталон технически недоступен до завершения рефлексии.

Переход на следующий этап происходит автоматически, когда отдельная структурированная проверка (не наставник, а отдельный вызов-судья по цели текущего этапа) подтверждает, что цель этапа реально закрыта. Кнопка «Этап готов» остаётся доступна всегда — ей можно форсировать переход раньше; если проверка на тот момент не подтвердила критерий, переход всё равно происходит, но фиксируется как принудительный с кратким пояснением, чего не хватило. Если сама проверка недоступна (например, нет ключа), поведение по умолчанию — тоже засчитать переход принудительным, а не заблокировать кнопку.

Заготовленные подсказки этапа показаны сразу все, каждая — под своим спойлером в сайдбаре; открывать их можно в любом порядке и без ограничения по числу — раскрывать список некуда, кончаться ему тоже нечем. Отдельно, прямо в чате, доступна контекстная подсказка: наставник формулирует её с учётом уже сказанного кандидатом, а не по шаблону.

Прогресс этапа хранит уровень самостоятельности `independent` / `hinted` / `explained` и признак принудительного перехода. Это сигнал для рекомендации повторения, а не штраф в итоговой рубрике. После рефлексии оценщик даёт formative feedback, затем открывается обычное сравнение с эталоном, где принудительно пройденные этапы показаны отдельно.
...
Учебный слой хранится отдельно в `internal/tasks/data/learning.yaml`: каталог концепций, учебные треки, пререквизиты и опциональные overrides для отдельных `task_id`. Условие и эталон там не дублируются. Для любой задачи без override строится безопасный blueprint из шести общих этапов, поэтому обучающий режим покрывает весь банк.
...
Полная история остаётся в SQLite, но не переотправляется модели на каждом ходу. Сервер хранит компактную рабочую память старой части диалога, оставляет последние 10 сообщений дословно, всегда закрепляет публичный бриф и передаёт только последнюю показанную ревизию доски. Summary обновляется пакетами, чтобы дополнительный LLM-вызов не происходил на каждом сообщении. Каждый ход собирает список сообщений заново в памяти (без сохранения сессии между ходами): скрытая история вызовов инструментов не должна бесконтрольно расти между ходами.

Оценщику и сравнению передаётся содержательный диалог без служебных сообщений и повторных дампов доски; актуальная каноническая схема идёт отдельным блоком.
...
Отдельная роль для режима обучения. Даёт короткую обратную связь, отмечает один пробел и задаёт один следующий вопрос. Не выбирает этап и не видит эталон. Сервер собирает phase-scoped контекст заново для текущего этапа; попытка ученика обязательна до перехода.

Решение о переходе на следующий этап принимает не наставник, а отдельный структурированный вызов-судья по цели этапа, срабатывающий после каждой реплики наставника (автопереход) и при нажатии «Этап готов» (проверка форсированного перехода).
...
- **F13.** Обучающий hub с треками, рекомендацией следующей задачи, восстановлением этапа и прогрессом по концепциям.
- **F14.** Учебная сессия с обязательной попыткой, лестницей подсказок и блокировкой эталона до рефлексии.
