...
Этапом и уровнем подсказки управляет сервер, не модель. Наставник получает публичный бриф, цель текущего этапа, учебные цели и доступ к фактам условия через `reveal_facts`, но не получает `preferred_solution` или gold diagram. Эталон технически недоступен до завершения рефлексии.

Прогресс этапа хранит уровень самостоятельности `independent` / `hinted` / `explained`. Это сигнал для рекомендации повторения, а не штраф в итоговой рубрике. После рефлексии оценщик даёт formative feedback, затем открывается обычное сравнение с эталоном.
...
Отдельная роль для режима обучения. Даёт короткую обратную связь, отмечает один пробел и задаёт один следующий вопрос. Не выбирает этап и не видит эталон. Сервер собирает phase-scoped контекст заново для текущего этапа; переход разрешён только после попытки ученика.
