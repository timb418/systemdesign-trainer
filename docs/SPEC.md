...
- Режимы v1: полный mock; дрилл по типу архитектуры; только уточнение требований; сравнение с эталоном после попытки.
...
Доступен после своей попытки по той же задаче (полный mock или дрилл). Показывает эталонный нарратив, trade-offs и эталонную схему рядом с попыткой кандидата. Агент может провести разбор отличий, не объявляя любое отклонение ошибкой.
...
Каталог типов — отдельная сущность, не случайные названия продуктов. Задача принадлежит одному или нескольким типам. Режим дрилла выбирает тип, затем задачу из этого множества.
...
- Если вопрос уместный, но в фактах карточки ответа нет — не выдумывает цифры и требования; просит кандидата сделать разумное предположение и дальше на него опираться. Когда кандидат назвал предположение — коротко оценивает, реалистично оно для такого сервиса или совсем мимо (если мимо — почему и попросить поправить порядок величины / scope).
- Не выдумывает противоречащие карточке цифры: нагрузка и скрытые требования только из `reveal_facts`.
